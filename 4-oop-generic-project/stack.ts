interface Stack<T> { // 제네릭을 이용할 때는 인터페이스나 함수, 클래스, 타입 정의 부분에서 <T> 정의 
...
class StackImpl<T> implements Stack<T> {
  ...
  constructor(private capacity: number) { // 허용사이즈 initial value 설정
  }
  get size() { // get으로 외부에서 볼 수 있게 정의(set이 없으므로 변경은 x)
    return this._size;
  }
